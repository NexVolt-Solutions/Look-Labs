...
extension _DailySkinCareRoutineFlow on DailySkinCareRoutineViewModel {
  Future<void> _pollUntilSkincareFlowCompleted(int seq) async {
    ...
    _notify();
    try {
      for (var i = 0; i < DailySkinCareRoutineViewModel._flowPollMaxRounds; i++) {
        await Future<void>.delayed(DailySkinCareRoutineViewModel._flowPollInterval);
        if (seq != _loadSeq) return;
        final flowRes = await DomainQuestionsRepository.instance.getDomainFlow(
          DailySkinCareRoutineViewModel._domainKey,
        );
        if (seq != _loadSeq) return;
        if (flowRes.success && flowRes.data is Map) {
          ...
          if (DailySkinCareRoutineViewModel._flowStatusIsComplete(st)) {
            _applyFlowMap(map);
            return;
          }
  ...
  Future<void> _loadSkincareRoutine() async {
    final seq = ++_loadSeq;
    _flowPollInProgress = false;
    _loading = true;
    _loadError = null;
    ...
    try {
      ...
      final albumRes = await ImageUploadRepository.instance.getAlbumImages(
        domain: DailySkinCareRoutineViewModel._domainKey,
      );
      if (seq != _loadSeq) return;
      ...
      if (flowRes.success && flowRes.data is Map) {
        ...
        _applyFlowMap(map);
        if (DailySkinCareRoutineViewModel._flowStatusIsProcessing(
          map['status']?.toString(),
        )) {
