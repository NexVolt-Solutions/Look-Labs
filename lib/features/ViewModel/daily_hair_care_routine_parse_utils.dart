...
Map<String, dynamic>? _mapFromLooseJson(dynamic v) {
...
List<Map<String, String>> _parseRoutineList(dynamic raw) {
  ...
  for (final e in raw) {
    ...
    final steps = m['steps'];
    var subtitle =
        (m['description'] ?? m['details'] ?? m['body'] ?? m['instruction'] ?? m['text'] ?? '')
            .toString()
            .trim();
    if (subtitle.isEmpty && steps is List) subtitle = steps.map((s) => '• $s').join('\n');
    if (subtitle.isEmpty) subtitle = (m['duration'] ?? '').toString().trim();
...
List<Map<String, dynamic>> _attrsToGridRows(Map<String, dynamic> attrs) {
  ...
  attrs.forEach((key, value) {
    ...
    if (value is Map) {
      ...
      if (label != null && label.isNotEmpty && (conf is num || conf != null)) {
        final row = <String, dynamic>{'title': _humanize(k), 'subTitle': label};
